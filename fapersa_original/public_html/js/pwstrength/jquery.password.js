...
digitalspaghetti.password = {	
	...
	'init': function (element, options) {
		...
		if (digitalspaghetti.password.options.whereToDisplay) {
			digitalspaghetti.password.attachWidget(digitalspaghetti.password.options.whereToDisplay);
		}else{
	...
	'calculateScore': function (word) {
		...
		for (var key in digitalspaghetti.password.rules) if (digitalspaghetti.password.rules.hasOwnProperty(key)) {
			...
			digitalspaghetti.password.width =  "96";
			...
			jQuery('.password-strength-bar').animate({opacity: 0.5}, 'fast', 'linear', function () {
				jQuery(this).css({'display': 'block', 'background-color': digitalspaghetti.password.strColor}).text(digitalspaghetti.password.strText);
